...
class Trainer(object):
    ...
    def train_epoch(self, epoch):
        ...
        for iter, (x, labels) in enumerate(self.train_loader.get_iterator()):
            ...
            x = torch.Tensor(x).to(self.args.device)  # [B L N C]   
            labels = torch.Tensor(labels).squeeze().transpose(1,2).to(self.args.device)  # [B N L]   
    ...
    def test(model,attModel, dataloader,ori_edge_index, ori_adj, scaler, min, max, logger, args):
        ...
        with torch.no_grad():
            ...
            for iter, (x, labels) in enumerate(dataloader.get_iterator()):
                x = torch.Tensor(x).to(args.device)  # [B L N C]   
                labels = torch.Tensor(labels).squeeze().transpose(1,2).to(args.device)  # [B N L]   
